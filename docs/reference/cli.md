...
These flags are a no-op — everyone is allowed, matching today's default — if no identity map is configured at all for the deployment. `pincer doctor` warns if a channel is configured (bot token set, or WhatsApp enabled without self-chat-only mode) but no identity map exists at all, since that means the channel has no access control whatsoever.
