...
#### UUIDv7 row identifiers (migrations 0017 and 0018)

Every identifier Pincer mints for a row of its own is now a UUIDv7 — time
ordered, generated by the writer, and the same shape in every table. The
fourteen `INTEGER PRIMARY KEY AUTOINCREMENT` keys and the six hand-minted
string keys (memories, entities, conversations, `telephony_calls.call_id`,
`telephony_turns.turn_id`, `call_threads.thread_id`) are converted, along with
the seven columns that reference them. On Postgres the columns are a native
`uuid`; on SQLite they are the canonical 36-character string.

Identifiers that come from somewhere else keep their type and their meaning:
Twilio CallSids, `pincer_user_id`, the derived session key, phone numbers, the
W3C trace and span ids, and the SHA-1 event key that collapses a provider's
retried callbacks.

**Upgrading**

- **Python 3.14 is now the minimum.** `uuid.uuid7()` is standard library from
  3.14, and taking it from there rather than a dependency moves the floor.
- **Existing rows keep their place in time.** Each row's new id carries the
  timestamp from that row's own creation column, so anything ordered by the
  key — a call's transcript, the newest messages — reads back unchanged.
- **Drain the task queue before deploying, or expect a warning.** A
  `run_scheduled_action` job enqueued beforehand carries an integer schedule
  id, which now names no row. The actor drops such a job with a warning naming
  migration 0017; that branch is meant to last one release.
- **`0017` downgrades lossily** — rows are renumbered rather than restored to
  their original integers, which is safe only because nothing references them.
  Back up first (`scripts/backup-db.sh`). `0018` downgrades cleanly.
- The empty-string sentinel on `voice_calls.thread_id`,
  `telephony_events.turn_id` and `telephony_spans.turn_id` is now NULL, which
  is what the column always meant. API responses are unchanged.
- Thread ids lose their `thr_` prefix. Nothing branched on it.
...
- **Twilio signature validation fails closed when uninitialized.** `verify_http_request` / `verify_ws_upgrade` now refuse (403 / close 1008) when called with `settings=None` — i.e. if the Twilio routers are ever mounted before or without `init_voice_routes` — instead of silently skipping validation. The normal startup order (`init_voice_routes` runs before the API server starts) is unaffected; this closes the one path where the Bearer-auth exemption for `/api/apps/twilio/*` could have stood in front of no auth at all. Enforcement tests now also cover `/transfer-result`, the legacy `/voice/relay-webhook` alias, and an end-to-end check that the middleware exemption hands off to a live signature check.
- **Voice sockets can no longer mint or hijack calls.** A `setup` (ConversationRelay `/relay`, Media Streams `/stream/{sid}`, or the `/relay-webhook` transport) for a call SID with no registered `CallState` and no outbound status record is now refused and audited instead of creating fresh inbound state from socket-supplied `callSid`/`from` — genuine inbound calls are always registered by the signature-verified `/webhook` before their TwiML (and hence any socket) exists, so an unknown SID is never a legitimate Twilio call. Previously a connection that got past WS auth (or a deployment with `PINCER_VOICE_WS_AUTH_REQUIRED=false` / no auth token) could open a conversation with the agent under an attacker-chosen caller identity, which feeds cross-channel identity resolution. Additionally, a `setup` naming a call whose socket is still connected is refused (close 1008) instead of overwriting `state.metadata["websocket"]` — overwriting redirected the agent's speech to the new socket and left the genuine caller permanently silent.

### Fixed

- **Voicemail/AMD hangups dropped the post-call report** — the `/status` handler sent its own "call ended" stage before `end_call`, which popped the call from status tracking and consumed the ENDED stage, so the structured report built by the post-call pipeline was silently discarded (and the report fell back to `state.*` for user/language). The handler now stashes the reason in `state.metadata["end_reason"]` (surfaced as a line in the report) and lets the pipeline send the final message; the direct short status remains only when no conversation state exists and hence no pipeline runs. Same fix for a `busy`/`no-answer`/`failed`/`canceled` callback that arrives while call state exists.
