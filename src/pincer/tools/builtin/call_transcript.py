...
async def get_call_transcript(call_sid: str = "") -> str:
            ...
            if not call_sid.strip():
                rows = list(
                    await db.execute_fetchall(
                        "SELECT call_sid, to_number, started_at FROM voice_calls ORDER BY started_at DESC LIMIT 1"
                    )
                )
                if not rows:
