...
async def test_create_doc_no_content(mock_factory, mock_docs_service):
    mock_docs_service.documents().create().execute.return_value = {"documentId": "new_doc", "title": "New Document"}
...
async def test_create_doc_with_content(mock_factory, mock_docs_service):
    ...
    result = await google__create_doc(mock_factory, title="Doc With Content", content="Hello, World!")
    assert "new_doc2" in result


async def test_insert_text(mock_factory, mock_docs_service):
    mock_docs_service.documents().batchUpdate().execute.return_value = {}
    result = await google__insert_text(mock_factory, document_id="doc1", text="New paragraph\n", index=1)
...
async def test_replace_text(mock_factory, mock_docs_service):
    ...
    result = await google__replace_text(mock_factory, document_id="doc1", find_text="foo", replace_text="bar")
    assert "3" in result
...
async def test_insert_table(mock_factory, mock_docs_service):
    mock_docs_service.documents().batchUpdate().execute.return_value = {}
    result = await google__insert_table(mock_factory, document_id="doc1", rows=3, columns=4)
    assert "3" in result
    assert "4" in result
...
async def test_add_comment(mock_factory, mock_docs_service):
    ...
    drive_svc = MagicMock()
    drive_svc.comments().create().execute.return_value = {"id": "comment1", "content": "Please review this section"}
    ...
    result = await google__add_comment(mock_factory, file_id="doc1", content="Please review this section")
